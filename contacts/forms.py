...
class ContactForm(FilteredModelForm):
    ...
    is_unsubscribed = forms.BooleanField(required=False)
    ...
    def __init__(self, *args, **kwargs):
        ...
        for field in allowed_fields:
            match field["type"]:
                case "str":
                    fields[f"fields__{field['name']}"] = forms.CharField(required=False, initial=self.custom_fields_values.get(field["name"]))
                case "number":
                    fields[f"fields__{field['name']}"] = forms.IntegerField(required=False, widget=forms.NumberInput(), initial=self.custom_fields_values.get(field["name"]))
                case "date":
                    fields[f"fields__{field['name']}"] = forms.DateField(required=False, initial=self.custom_fields_values.get(field["name"]))
                case "bool":
                    fields[f"fields__{field['name']}"] = forms.BooleanField(
                        required=False, widget=forms.CheckboxInput(), initial=self.custom_fields_values.get(field["name"])
                    )
